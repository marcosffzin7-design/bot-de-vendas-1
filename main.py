...
async def painel(interaction: discord.Interaction):
    ...
    await interaction.response.defer(ephemeral=True)
    e = discord.Embed(title="⚙️ Painel", color=0x00FF00)
    v = discord.ui.View()
    ...
    async def bc_cb(i):
        m = discord.ui.Modal(title="Mensagem em Massa")
        msg = discord.ui.TextInput(label="Mensagem", style=discord.TextStyle.paragraph)
        async def s(it):
            ...
            for u in users:
                try: 
                    ...
                    await user.send(msg.value)
                    count += 1
                except: pass
            await it.followup.send(f"✅ Enviado para {count} usuários!", ephemeral=True)
